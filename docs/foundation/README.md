...
长期模块按经营问题划分，不按后台页面划分：

| 领域模块 | 典型数据源 |
| --- | --- |
| 服务域 `service` | 新灯塔 `new_lighthouse`、售后后台、手填数据 |
| 商品成长域 `product_growth` | 商品成长后台、商品列表 |
| 交易漏斗域 `trade_funnel` | 生意参谋首页核心看板 |
| 找工厂/定制域 `factory_custom` | 找工厂后台 |
| 老客复购域 `customer_repeat` | 生意参谋、手填数据 |
| 风险护栏域 `guardrail` | 内部利润表、售后后台、找工厂后台 |

`DomainModule` 是从底层表派生的读取视图，只包含指标、数据源、诊断规则、动作模板和风险护栏，不包含当前读数。当前读数后续由数据录入、XLS 导入或页面复制进入。
